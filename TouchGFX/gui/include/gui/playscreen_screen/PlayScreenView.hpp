...
protected:
    ...
    uint16_t timeAnimation = 6;

    // CO DAM BAO logic "di tru" widget (remove khoi containerGocij, add vao containerij)
    ...
    // Neu khong co co nay, lan vao lai thu 2 se add lai widget DA CO SAN trong containerij/container1,
    ...
    // CO bao cho tearDownScreen() biet KHONG duoc luu game lan nay, dung khi nguoi dung
    // bam "No" trong saveGameContainer (muon roi man hinh ma KHONG luu tien trinh).
    // Neu khong co co nay, tearDownScreen() se tu dong luu binh thuong (vi logic mac dinh
    ...
    // nâng cấp version2.1; tạo ánh xạ i,j-> box_i_j;
    ...
    bool dichuyenGa = false; // true khi dang trong 1 luot di chuyen/animation => dung de chan input/Yes/No/Close, tranh xung dot trang thai
